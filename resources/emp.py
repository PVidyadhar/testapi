...
class Emp(Resource):
    ...
    def post(self):
        ...
        parser.add_argument("sal",type=str,help="sal cannot be left blank!")
        parser.add_argument("comm",type=str)
        parser.add_argument("deptno",type=int,required=True,help="comm cannot be left blank!")
        ...
        if data['comm']!=None:
            try:
                query(f"""INSERT INTO testapi.emp VALUES({data['empno']},'{data['ename']}','{data['job']}',{data['mgr']},'{data['hiredate']}','{data['sal']}','{data['comm']}',{data['deptno']},'{data['pass']}');""")
            ...
            try:
                query(f"""INSERT INTO testapi.emp(empno, ename, job, mgr, hiredate, sal, deptno, pass) VALUES({data['empno']},'{data['ename']}','{data['job']}',{data['mgr']},'{data['hiredate']}','{data['sal']}',{data['deptno']},'{data['pass']}');""")
